...
const ExploreScreen = () => {
  ...
  const extractDetail = (description: string, key: string): string | null => {
    const regex = new RegExp(`${key}\\s*[:：]\\s*(.+)`, "i");
    ...
    <>
      ...
      <ScrollView className="p-4 bg-gray-200 mb-2">
        {products.map((item, index) => {
          const lifespan = extractDetail(item.description, "lifespan");
          const lighting = extractDetail(item.description, "lighting");
          const color = extractDetail(item.description, "color");
