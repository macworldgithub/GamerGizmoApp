...
const ProductDetail = () => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const response = await axios.get(`${API_BASE_URL}/products/getProductById?id=${id}`);
    ...
    <ScrollView className="p-4 bg-white">
      ...
      <View className="border-t border-b border-gray-200">
        ...
        <View className="flex-row justify-between items-center py-3 px-4">
          ...
          <Text className="text-gray-500">{product.condition_product_conditionTocondition?.name || "N/A"}</Text>
        </View>
        <View className="flex-row justify-between items-center py-3 px-4">
          <Text className="text-gray-900 font-semibold">Posted On</Text>
          <Text className="text-gray-500">{dayjs(product.created_at).fromNow()}</Text>
        </View>
      </View>
      ...
      <View className="mt-6">
        ...
        <View className="flex-row">
          <Text className="text-gray-700 flex-1">{product.description || "No description provided."}</Text>
        </View>
      </View>
      ...
      <View className="mt-6 p-4 border border-gray-300 rounded-md bg-gray-50 shadow-sm">
        ...
        <View className="flex-row items-center">
          <Image
            source={{ uri: `${API_BASE_URL}${product.seller?.avatar || '/default-avatar.png'}` }}
            className="w-12 h-12 rounded-full mr-4"
          />
          <View>
            <Text className="font-semibold text-gray-800">{product.seller?.name || "Seller"}</Text>
